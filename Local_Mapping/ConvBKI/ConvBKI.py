...
class ConvBKI(torch.nn.Module):
    ...
    def calculate_kernel(self, i=0):
        return self.sparse_kernel(self.kernel_dists_z, torch.clamp(self.ell_z[i], min=0.01), self.sigma) * \
                                 self.sparse_kernel(self.kernel_dists_h, torch.clamp(self.ell_h[i], min=0.01), self.sigma)
    ...
    def add_to_update(self, update, grid_pc, continuous=False):
        if continuous:
            ...
            feat_out = torch.zeros(xyz.size(0), feat.size(1), dtype=feat.dtype, device=self.device)
