...
"""
def talker(base_fp, pkg_name, start_idx=0):
    ...
    rellis_ds = Rellis3dDataset(directory=base_fp, device=device, 
        num_frames=10, remap=True, use_aug=False, use_gt=False, model_setting="test")
    
    for idx in range(start_idx, len(rellis_ds)):
        ...
        # new_pc = np.dot(pose_mat[:3, :3], pc_np.T).T + pose_mat[:3, 3]
        new_pc = pc_np
...
if __name__ == '__main__':
    ...
    talker(BASE_DIR, pkg_name="semantic_bki", start_idx=824)
